...
export function createLoginForPerson(actor: Actor, personId: string, email: string, password: string): User {
  ...
  if (password.length < 4) throw new RuleError("New password must be at least 4 characters.");
  const user: User = {
...
export function createPerson(actor: Actor, input: PersonInput, login?: { email: string; password: string }): Person {
  ...
  if (login) {
    ...
    if (login.password.length < 4) throw new RuleError("New password must be at least 4 characters.");
  }
  const person: Person = {
...
export function updateOwnProfile(actor: Actor, patch: Partial<Pick<Person, "name" | "email" | "phone" | "notifyEmail" | "notifySms" | "appearance">>): Person {
  ...
  Object.assign(p, {
    ...
    ...(patch.appearance !== undefined ? { appearance: { ...(p.appearance ?? { fontSize: "default", density: "comfortable", photoUrl: null }), ...patch.appearance } } : {}),
  });
